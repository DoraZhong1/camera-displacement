...
from camera_displacement.video_io import VideoError, VideoLoader
...
def main(argv=None) -> int:
    ...
    if args.roi:
        ...
        roi = select_roi_interactive(baseline)
        if roi is None:
    ...
    print("\n=== Absolute displacement (vs baseline) — primary result ===")
    print(f"  Frames analyzed:      {s.get('frames', 0)}")
    print(f"  OK / low-conf / lost: {s.get('ok_frames', 0)} / "
          f"{s.get('low_confidence_frames', 0)} / {s.get('lost_frames', 0)}")
    print(f"  Max total displacement: {s.get('max_total_displacement_px', 0):.2f} px "
          f"(frame {s.get('max_total_at_frame', 0)})")
    print(f"  Mean total displacement: {s.get('mean_total_displacement_px', 0):.2f} px")
    if calibration.is_calibrated:
        mm = calibration.px_to_mm(s.get("max_total_displacement_px", 0.0))
        print(f"  Max total displacement: {mm:.3f} mm")
